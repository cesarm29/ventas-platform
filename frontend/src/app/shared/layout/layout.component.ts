        ...
        <div class="p-6 border-b border-dark-700/50">
          <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-xl bg-primary-600 flex items-center justify-center">
              <svg class="w-5 h-5 text-dark-50" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 100 4 2 2 0 000-4z"/>
              </svg>
            </div>
            <div>
              <h1 class="text-lg font-bold text-dark-50">VentasPlatform</h1>
        ...
        <header class="sticky top-0 z-20 h-16 bg-dark-900/80 backdrop-blur-xl border-b border-dark-700/50 flex items-center px-4 lg:px-6 gap-4">
          ...
          <div class="hidden sm:flex items-center gap-2">
            <div class="w-8 h-8 rounded-full bg-primary-600/10 flex items-center justify-center">
              <span class="text-primary-400 text-xs font-semibold">{{ (name$ | async)?.charAt(0)?.toUpperCase() }}</span>
            </div>
            <span class="text-sm text-dark-300 max-w-[120px] truncate">{{ name$ | async }}</span>
          </div>
          <button (click)="theme.toggle()" class="w-9 h-9 rounded-full flex items-center justify-center text-dark-400 hover:text-dark-50 hover:bg-dark-700/50 transition-colors" [title]="theme.darkMode() ? 'Light Mode' : 'Dark Mode'">
            @if (theme.darkMode()) {
          ...
          <button (click)="logout()" class="ml-auto w-9 h-9 rounded-full flex items-center justify-center text-dark-400 hover:text-red-400 hover:bg-dark-700/50 transition-colors" title="Cerrar sesion">
